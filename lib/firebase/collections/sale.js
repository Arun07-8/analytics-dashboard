...
export const getAllSales = async () => {
  try {
    const querySnapshot = await getDocs(query(collection(db, 'sales'), where('isDeleted', '==', false)));
...
export const getSalesByCustomer = async (customerId) => {
  try {
    const q = query(collection(db, 'sales'), where('customerId', '==', customerId), where('isDeleted', '==', false));
...
export const getSalesByStaff = async (staffId) => {
  try {
    const q = query(collection(db, 'sales'), where('staffId', '==', staffId), where('isDeleted', '==', false));
    const querySnapshot = await getDocs(q);
    return querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  } catch (error) {
