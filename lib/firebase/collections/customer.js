...
export const getCustomerByEmail = async (email) => {
  try {
    const q = query(collection(db, 'customers'), where('email', '==', email), where('isDeleted', '==', false));
...
export const getCustomerByMobile = async (mobile) => {
  try {
    const q = query(collection(db, 'customers'), where('mobile', '==', mobile), where('isDeleted', '==', false));
    const querySnapshot = await getDocs(q);
    if (querySnapshot.empty) return null;
    return { id: querySnapshot.docs[0].id, ...querySnapshot.docs[0].data() };
  } catch (error) {
