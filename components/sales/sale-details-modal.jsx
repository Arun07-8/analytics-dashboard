                    ...
                    {!isRejected && (
                        <div className="absolute right-8 top-8 z-20">
                            <Button
                                variant="outline"
                                size="sm"
                                className="bg-background/10 border-background/20 text-background hover:bg-background/20 hover:text-background font-semibold text-xs gap-2 h-9 px-4 rounded-xl backdrop-blur-md"
                    ...
                    <div className="relative z-10">
                        <DialogHeader className="space-y-1">
                            <div className="flex items-center gap-2 opacity-60 mb-2">
                                ...
                                <span className="text-[10px] font-semibold uppercase tracking-[0.2em]">Transaction Receipt</span>
                            </div>
                ...
                <div className="p-8 space-y-10 bg-background">
                        ...
                        <div className="bg-red-50 border border-red-100 rounded-[2rem] p-1 mb-8 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-700 shadow-md">
                            <div className="bg-red-600 px-8 py-3.5 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-white">Official Rejection Notice</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="h-1.5 w-1.5 rounded-full bg-white animate-pulse" />
                                    <span className="text-[10px] font-semibold uppercase tracking-widest text-white/80">Action Required</span>
                                </div>
                            </div>
                            ...
                            <div className="p-6 bg-white">
                                <div className="relative">
                                    ...
                                    <div className="pl-6">
                                        <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground mb-2">Message from Administrator</p>
                                        <h2 className="text-xl font-bold text-red-600 tracking-tight leading-snug italic">
                                            "{sale.declineReason}"
                                        </h2>
                                        <div className="mt-4 flex items-center gap-4">
                                            <div className="h-[1px] flex-1 bg-red-50" />
                                            <p className="text-[10px] font-semibold text-red-600/40 uppercase tracking-[0.2em]">Ref: {sale.salesRefId?.[0] || "SALE_OBJ"}</p>
                                            <div className="h-[1px] flex-1 bg-red-50" />
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-muted/50 p-6 rounded-2xl border border-border/50">
                                <h4 className="font-bold text-xl mb-3">{customer?.name || sale.customerName}</h4>
                            ...
                            <div className="bg-muted/50 p-6 rounded-2xl border border-border/50 flex flex-col justify-between h-[calc(100%-24px)]">
                                <div className="flex justify-between items-start mb-6">
                                    ...
                                    <div className="text-right">
                                        <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1">Staff Member</p>
                                        <p className="font-bold text-sm">{sale.staffName}</p>
                                    </div>
                                </div>
                                <div className="pt-4 border-t border-border/50">
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Timestamp</span>
                        ...
                        <div className="rounded-2xl border border-border overflow-hidden">
                            <Table>
                                <TableHeader className="bg-muted/50">
                                    <TableRow>
                                        <TableHead className="py-4 px-6 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Description</TableHead>
                                        <TableHead className="text-right py-4 px-6 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Rate</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {sale.services?.map((service, index) => (
                                        <TableRow key={index} className="hover:bg-transparent border-b border-border/50 last:border-0">
                                            ...
                                            <TableCell className="text-right py-5 px-6 font-bold tabular-nums">
                    ...
                    <div className="bg-muted/30 p-8 rounded-3xl space-y-4 border border-border/50">
                        <div className="flex justify-between text-muted-foreground">
                            <span className="text-[10px] font-semibold uppercase tracking-widest">Subtotal Amount</span>
                            <span className="font-bold tabular-nums">₹{(Number(sale.totalAmount) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex justify-between text-emerald-600">
                            <span className="text-[10px] font-semibold uppercase tracking-widest">Payment Received</span>
                            <span className="font-bold tabular-nums">₹{(Number(sale.paidAmount) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                        </div>
                        <Separator className="bg-border" />
                        <div className="flex justify-between items-center pt-2">
                            <div className="flex flex-col">
                                <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground mb-1">Financial Balance</span>
                                <Badge variant="secondary" className={`text-[9px] font-semibold uppercase tracking-widest w-fit rounded-full px-2 py-0 ${balance > 0 ? "text-destructive bg-destructive/5" : "text-emerald-500 bg-emerald-500/5"}`}>
                                    {balance > 0 ? "Debit Balance" : "Account Cleared"}
                                </Badge>
                            </div>
                            <div className={`text-3xl font-bold tracking-tight tabular-nums ${balance > 0 ? "text-destructive" : "text-emerald-500"}`}>
